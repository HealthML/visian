...
export class MiaReviewStrategy extends ReviewStrategy {
  public static async fromDataset(store: RootStore, datasetId: string) {
    const images = await getImagesByDataset(datasetId);
    return new MiaReviewStrategy(store, images);
  }

  public static async fromJob(store: RootStore, jobId: string) {
    const images = await getImagesByJob(jobId);
    return new MiaReviewStrategy(store, images, jobId);
  }

  public static async fromImageIds(store: RootStore, imageIds: string[]) {
  ...
  constructor(store: RootStore, images: Image[], jobId?: string) {
    super(store);
    this.images = images;
